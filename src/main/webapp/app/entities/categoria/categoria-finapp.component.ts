...
@Component({
    selector: 'jhi-categoria-finapp',
    templateUrl: './categoria-finapp.component.html'
})
export class CategoriaFinappComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/categoria-finapp'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/categoria-finapp', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: CategoriaFinapp) {
        return item.id;
    }
