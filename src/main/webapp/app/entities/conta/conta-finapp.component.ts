...
@Component({
    selector: 'jhi-conta-finapp',
    templateUrl: './conta-finapp.component.html'
})
export class ContaFinappComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/conta-finapp'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/conta-finapp', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ContaFinapp) {
        return item.id;
    }
